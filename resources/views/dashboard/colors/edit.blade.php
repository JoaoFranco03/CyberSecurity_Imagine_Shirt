    ...
    <main>
        ...
        <div>
            <div
                ...
                <form id="add_color_form" action="{{route('colors.update', ['color' => $color])}}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="">
                        <div class="mt-3 text-center sm:mt-0 sm:text-left">
                            <div>
                                <label for="name"
                                    ...
                                    Name:</label>
                                <input type="text" name="name" id="color_name" placeholder="{{$color->name}}"
                            ...
                            <div class="mt-4">
                                <label for="code"
                                       class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Color
                                    Code:</label>
                                <input type="text" name="code" id="color_code" placeholder="{{$color->code}}"
                                       onkeydown="document.getElementById('color_selector').value= '#' + document.getElementById('color_code').value +'0' "
                                       class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white">
                                @error('code')
                                <div class="text-red-600 mt-1 text-md font-bold">
                                    {{ $message }}
                    ...
                    <div class="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                        ...
                        <a href="{{route('colors.index')}}" type="button" data-modal-hide="color-modal"
                ...
                <a class=" flex items-center" href="#">
                    <input
                        class="relative inline-flex rounded-lg h-36 w-36 ml-16 aspect-1 border border-gray-300"
                        value="#{{$color->code}}" type="color" id="color_selector" onchange="document.getElementById('color_code').value=document.getElementById('color_selector').value.substring(1)">
                </a>
            </div>
        </div>
